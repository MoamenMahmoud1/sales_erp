        ...
        async with asyncio.timeout(timeout_s):
            ...
            for line in header_lines[1:]:
                if b":" in line:
                    ...
                    headers[key.decode("ascii", "ignore").lower()] = value.strip().decode(
                        "ascii", "ignore"
                    )
...
async def run_benchmark(
    *,
    ...
    try:
        ...
        print(f"DEADLINE_EXCEEDED completed={len(results)}/{requests} elapsed={elapsed:.3f}s")
        raise
    return results
...
def main() -> None:
    ...
    try:
        ...
        pass
    elapsed = time.perf_counter() - started
    report(results, args.requests, elapsed)
