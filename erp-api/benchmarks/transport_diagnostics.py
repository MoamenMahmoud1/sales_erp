...
EXPECTED = {
    "stack": os.environ.get("BENCH_EXPECT_STACK"),
...
async def run():
    ...
    stack_verified = bool(EXPECTED.get("stack")) and all_expected and not errors.get("stack_mismatch")
        ...
        json.dumps(
            {
                ...
                "transport_minus_server_p50_ms": max(
                    pct(total_ms, 0.50) - pct(server_ms, 0.50), 0
                ) if total_ms and server_ms else None,
                "transport_minus_server_p99_ms": max(
                    pct(total_ms, 0.99) - pct(server_ms, 0.99), 0
                ) if total_ms and server_ms else None,
