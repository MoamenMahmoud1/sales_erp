...
class _CarTripEditorPageState extends State<CarTripEditorPage> {
  ...
  // Keep the status constants close to the editor without spreading status
  // handling across individual field callbacks.
            ...
            InkWell(
              ...
              child: InputDecorator(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: _dueDate == null ? scheme.onSurfaceVariant : scheme.onSurface),
                ),
              ),
            ),
          ],
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Product discount %', suffixText: '%', border: OutlineInputBorder()),
                    onChanged: (value) {
                      final parsed = double.tryParse(value);
                      if (parsed != null) setState(() => _discounts[id] = parsed.clamp(0, 100).toDouble());
                    },
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 3),
                      Text(_money(line.netValue), style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w900)),
                    ],
                  ),
                ),
              ],
        ...
        Expanded(
          child: TextFormField(
            ...
            onChanged: (raw) {
              ...
              onChanged(parsed.clamp(0, max ?? 1000000));
            },
          ),
        ),
          ...
          children: [
            Expanded(
              child: TextFormField(
                ...
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: const InputDecoration(labelText: 'Global discount %', suffixText: '%', border: OutlineInputBorder()),
                onChanged: (raw) {
                  final parsed = double.tryParse(raw);
                  if (parsed != null) setState(() => _globalDiscountPercent = parsed.clamp(0, 100).toDouble());
                },
              ),
            ),
  ...
  Widget _summaryRow(String label, CarMoney amount, bool accent) => Padding(
        padding: const EdgeInsets.symmetric(vertical: 3),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(label),
            Text(_money(amount), style: TextStyle(fontWeight: FontWeight.w700, color: accent ? Theme.of(context).colorScheme.primary : null)),
          ],
        ),
      );
