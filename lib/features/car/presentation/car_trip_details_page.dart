...
class _CarTripDetailsPageState extends State<CarTripDetailsPage> {
  ...
  ({StatusType type, String label}) _status(CarTrip trip) {
    ...
    return switch (status) {
      ...
      CarPaymentStatus.partiallyPaid => (type: StatusType.warning, label: 'Partially paid'),
        ...
        children: [
          Row(children: [
            Container(
              ...
              child: Icon(Icons.local_shipping_rounded, color: scheme.primary, size: 29),
            ),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(trip.displayNumber, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text('${trip.salesCarName} · ${trip.warehouseName}', style: TextStyle(color: scheme.onSurfaceVariant)),
            ])),
          ...
          Row(children: [
            ...
            Expanded(child: _info('Closed', trip.closedAt == null ? 'Open' : _formatDate(trip.closedAt!))),
            Expanded(child: _info('Due', trip.dueDate == null ? '—' : _formatDate(trip.dueDate!))),
          ]),
        ],
      ),
    );
  ...
  Widget _flowMetric(String label, int value, IconData icon, Color color) => Expanded(child: Column(children: [
          ...
          Padding(
            ...
            child: Column(children: [
              Row(children: [
                ...
                Text(_money(line.netValue.minorUnits), style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w900)),
              ]),
              const SizedBox(height: 4),
              Row(children: [
                Expanded(child: Text('${line.item.loadedCartons} loaded · ${line.item.returnedCartons} returned · ${line.soldCartons} sold', style: TextStyle(fontSize: 11, color: scheme.onSurfaceVariant))),
                Text('${line.item.discountPercent.toStringAsFixed(2)}% off', style: TextStyle(fontSize: 11, color: scheme.onSurfaceVariant)),
              ]),
              const SizedBox(height: 4),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('${_money(line.item.unitPrice.minorUnits)} / carton', style: TextStyle(fontSize: 11, color: scheme.onSurfaceVariant)),
                Text('Gross ${_money(line.grossValue.minorUnits)} · Discount ${_money(line.discountAmount.minorUnits)}', style: TextStyle(fontSize: 11, color: scheme.onSurfaceVariant)),
              ]),
  ...
  Widget _financialCard(dynamic summary) => AppCard(
        child: Column(children: [
          const Align(alignment: Alignment.centerLeft, child: Text('Financial summary', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900))),
          ...
          _amountRow('Global discount (${summary.globalDiscountPercent.toStringAsFixed(2)}%)', summary.globalDiscountAmount),
  ...
  Widget _paymentCard(CarTrip trip, dynamic remaining, CarPaymentStatus status) {
    ...
    final days = _evaluator.daysOverdue(trip, DateTime.now(), summary: _calculator.summary(trip));
    return AppCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Expanded(child: Text('Payment', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900))), StatusBadge(type: statusType, label: statusLabel)]),
        const SizedBox(height: 12),
        Row(children: [
  ...
  Widget _amountRow(String label, dynamic amount, {bool strong = false}) => Padding(
