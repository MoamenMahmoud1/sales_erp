...
/// Today contains finalized trips only. The date is the confirmation/closing
/// date so a draft saved yesterday cannot appear in Today until confirmed.
...
class _CarDailyDashboardPageV4State extends State<CarDailyDashboardPageV4>
  ...
  void _onTripChanged(CarTrip trip) {
    ...
    final confirmedAt = trip.closedAt?.toLocal();
    if (trip.status == CarTripStatus.closed &&
        confirmedAt != null &&
      ...
      child: ListView(
        ...
        children: [
          ...
          _summaryCard(
            ...
            subtitle:
                '${_trips.length} finalized trip${_trips.length == 1 ? '' : 's'}',
