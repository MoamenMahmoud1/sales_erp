...
class _CarTripsPageState extends State<CarTripsPage> {
  ...
  bool _matchesCurrentFilter(CarTripSummaryView view) {
    ...
    return switch (_tab) {
      ...
      _TripFilterTab.paid => view.paymentStatus(_evaluator, DateTime.now()) == CarPaymentStatus.paid,
      _TripFilterTab.partial => view.paymentStatus(_evaluator, DateTime.now()) == CarPaymentStatus.partiallyPaid,
      _TripFilterTab.unpaid => view.paymentStatus(_evaluator, DateTime.now()) == CarPaymentStatus.unpaid,
      _TripFilterTab.overdue => view.paymentStatus(_evaluator, DateTime.now()) == CarPaymentStatus.overdue,
    };
  }
        ...
        child: ListView(
          ...
          children: [
                  ...
                  for (final tab in _TripFilterTab.values)
                    Padding(
                      ...
                      child: ChoiceChip(
                        ...
                        selected: _tab == tab,
                        onSelected: (_) { setState(() => _tab = tab); _load(); },
                      ),
                    ),
                ],
              ),
            ...
            if (_loading)
              ...
              EmptyState(icon: Icons.error_outline_rounded, title: 'Unable to load Car trips', message: _error!, actionLabel: 'Retry', onAction: _load)
            else if (_trips.isEmpty)
              const EmptyState(icon: Icons.receipt_long_outlined, title: 'No Car trips found', message: 'Create a trip or change the current filters.')
            else
  ...
  Widget _buildTripCard(CarTripSummaryView trip) {
    ...
    return Padding(
      ...
      child: AppCard(
        ...
        child: Row(
          children: [
            Container(width: 52, height: 52, decoration: BoxDecoration(color: scheme.primaryContainer, borderRadius: BorderRadius.circular(17)), child: Icon(Icons.local_shipping_rounded, color: scheme.primary)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(_tripTitle(trip.openedAt), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900)),
