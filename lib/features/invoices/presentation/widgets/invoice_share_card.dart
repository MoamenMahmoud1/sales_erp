...
/// بيانات الفاتورة المستخدمة في كارت المشاركة.
class ShareInvoiceData {
  ...
  static ShareInvoiceData fromMap(Object? header, List<ShareInvoiceItem> items) {
    final m = Map<String, Object?>.from(header as Map);
    return ShareInvoiceData(
      ...
      discount: (m['coupon_discount'] as num).toDouble(),
      total: (m['total'] as num).toDouble(),
      items: items,
      paidStatus: (m['payment_status'] as String?) ?? 'unpaid',
      paymentMethod: (m['payment_method'] as String?) ?? 'cash',
      paidAmount: (m['paid_amount'] as num?)?.toDouble() ?? 0,
    );
  }
}
/// كارت فاتورة نضيف يرسم كصورة ويُشارك.
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [scheme.primaryContainer.withValues(alpha: .55), scheme.surface],
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Total', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: scheme.primary)),
              Text(_money(data.total),
                  style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: scheme.primary)),
            ],
          ),
          const SizedBox(height: 14),
          Center(
...
Widget _totalLine(String label, double amount, ColorScheme scheme,
